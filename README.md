...
| 🔎 **OCR 智能文字识别** | 基于 rapidocr-onnxruntime-lite（仅 15MB），实时识别 `跳跃`/`继续`/`再次挑战` 等 UI 文字 |
| ⌨️ **裸 Win32 SendInput** | ctypes 直接调用系统 API 模拟键鼠，**零第三方依赖**，pynput / pyautogui 统统不需要 |
| 🖥️ **Win11 风格 GUI 状态面板** | tkinter 渲染简约状态卡片，实时显示战斗次数/管理员权限/异常诊断 |
| 🔥 **全局热键一键启停** | F1 开始 / F2 停止，**即使焦点不在 GUI 窗口也能响应** |
| 📊 **战斗次数统计** | 运行状态卡片实时显示已战斗次数，一目了然 |
| 🩺 **智能异常诊断** | 报错自动归类 OCR/窗口/权限等问题，附一键复制堆栈 |
| 📝 **日志文件回溯** | 每次运行生成 `logs/gbfr.log` |
| 🧩 **多屏 DPI 自适应** | Per-Monitor DPI 感知 + 负坐标多屏回退，4K / 异形分辨率全覆盖 |
| 🗜️ **Nuitka 原生编译** | anti-bloat + 排除 20+ stdlib + 无控制台窗口 + zstd 压缩，嵌入 15MB 模型后仍极致小巧 |
...
1. **右键 → 以管理员身份运行** `GBFR_AutoReBattle.exe`（程序会自动提权，若失效请手动右键运行）
2. 启动后弹出 **Win11 风格状态监控面板**，显示管理员权限、运行状态和操作说明
3. 按下 **`F1`** 开始自动循环，或点击面板上的 **▶ 启动战斗** 按钮
